                    ...
                    if not conn_response["err_status"]:
                        ...
                        if self.import_task or self.export_task:
                            _task = task.Task(
                                ...
                                initial_date=self.custom_from_datetime, end_date=self.custom_to_datetime, folder=None)
                            if self.import_task:
                        ...
                        if self.import_email:
                            _gmail = gmail.GMail(
                                gl_access_token=conn_response["response"], self_env=self.env, user_model=self.env.user,
                                default_email=None, initial_date=self.custom_from_datetime,
                                end_date=self.custom_to_datetime)
                            gmail_response = _gmail.import_mails()
                            if not gmail_response["err_status"]:
                                imp_mail += gmail_response["success"]
...
class MailMailContact(models.Model):
    ...
    def send_mail(self, js_data):
        sd_resp = {"err_status": True, "response": None}
        try:
            credentials = self.env[constants.GOOGLE_CREDENTIALS_MODEL].get_google_credentials()
            if constants.RESPONSE_ERROR_KEY not in credentials and \
                    constants.RESPONSE_ERR_MESSAGE_KEY not in credentials:
                conn = connection.Connection(google_app_cred=credentials, default_env=self.env)
                response = conn.get_msv_access_token()
                if not response["err_status"]:
                    _gmail = gmail.GMail(
                        gl_access_token=response["response"], self_env=self.env, user_model=self.env.user)
                    serv_response = _gmail.send_mail(json_params=js_data)
                    if not serv_response["err_status"]:
                        sd_resp["err_status"] = False
                    else:
                        sd_resp["response"] = "Log >> " + str(serv_response["response"])
                else:
                    sd_resp["response"] = "Log >> " + str(response["response"])
            else:
                sd_resp["response"] = "Log >> " + str(credentials["err_message"])
        except Exception as ex:
            sd_resp["response"] = "Log >> Oops, Mail could not be sent: " + str(ex)
        return sd_resp
    ...
    def create(self, vals):
        ...
        try:
            ...
            if partner or len(partner) > 0:
                contact = self.env[constants.CONTACT_TASK_MODEL].search([("id", "=", partner[0])])
                if contact and len(contact) > 0:
                    _sr_data["emails"].append({"email": contact.email})
...
class GoogleDriveConnect(models.Model):
    ...
    def google_drive_upload_btn(self):
        ...
        if constants.RESPONSE_ERROR_KEY not in credentials and constants.RESPONSE_ERR_MESSAGE_KEY not in credentials:
            ...
            conn_response = connect.get_msv_access_token()
            if not conn_response["err_status"]:
                _onedrive = gdrive.GDrive(gl_access_token=conn_response["response"])
    ...
    def google_drive_download_btn(self):
        ...
        if constants.RESPONSE_ERROR_KEY not in credentials and constants.RESPONSE_ERR_MESSAGE_KEY not in credentials:
            ...
            if not conn_response["err_status"]:
                _onedrive = gdrive.GDrive(gl_access_token=conn_response["response"])
                _dr_response = _onedrive.import_drive_documents(db_cursor=self.env.cr, user_rcd=r_partner,
                                                                self_env=self.env)
                if not _dr_response["err_status"]:
...
class GoogleResPartner(models.Model):
