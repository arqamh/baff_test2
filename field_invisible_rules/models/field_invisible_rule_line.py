...
class FieldInvisibleRuleLine(models.Model):
    _name = 'field.invisible.rule.line'
    _description = 'Field Invisible Rule Line'

    rule_id = fields.Many2one(
        'field.invisible.rule', string='Rule', required=True,
        ondelete='cascade', index=True)

    model_id = fields.Many2one(related='rule_id.model_id')

    field_id = fields.Many2one(
        'ir.model.fields', string='Field to Hide', required=True,
        ondelete='cascade',
        domain="[('model_id', '=', model_id), ('store', '=', True), "
               "('compute', '=', False)]",
        help='Field that becomes hidden when the trigger condition is met.')
    ...
    trigger_field_id = fields.Many2one(
        ...
        help='Field on the same model whose value triggers the hiding.')
    ...
    trigger_values = fields.Char(
        ...
        help='Comma-separated values that hide the field.')

    override_group_ids = fields.Many2many(
        'res.groups',
        'field_invisible_rule_line_group_rel', 'line_id', 'group_id',
        string='Override Groups',
        help='Users belonging to any of these groups still see the field.')
    ...
    def _check_trigger_values(self):
        Rule = self.env['field.invisible.rule']
        for line in self:
    ...
    def create(self, vals_list):
        ...
        for line in lines:
            ...
            line.rule_id.message_post(
                body=_("Added hidden field — %s", details))
        return lines
    ...
    def write(self, vals):
        ...
        for line in self:
            ...
            if changes:
                line.rule_id.message_post(
                    body=_("Updated hidden field <b>%(field)s</b> — %(changes)s",
    ...
    def unlink(self):
        ...
        for line in self:
            ...
            rule_messages.append((
                line.rule_id,
                _("Removed hidden field — Field: <b>%(field)s</b> / "
