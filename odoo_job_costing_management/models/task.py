...
class ProjectTask(models.Model):
    ...
    def _compute_stock_picking_moves(self):
        """FIXED: Removed ensure_one() crash, use |= to accumulate instead of overwriting"""
        for rec in self:
            all_lines = self.env['material.purchase.requisition.line']
            for picking in rec.picking_ids:
                all_lines |= picking.requisition_line_ids
            rec.move_ids = all_lines
    ...
    def view_stock_moves(self):
        self.ensure_one()
        # OPTIMIZED: single mapped() chain instead of N+1 loop
        stock_move_ids = self.move_ids.mapped('requisition_id.delivery_picking_id.move_ids').ids
        result = self.env["ir.actions.actions"]._for_xml_id("stock.stock_move_action")
        result['domain'] = str([('id', 'in', stock_move_ids)])
        return result
    ...
    def action_subtask(self):
        ...
        res['context'].update({'default_parent_task_id': self.id})
        return res
